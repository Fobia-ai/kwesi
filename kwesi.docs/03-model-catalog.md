...
- Inputs: text prompt (free-text style description, no language restriction), structured **lyrics in 50+ languages** (real, verified — see below), reference audio (style/cover), duration (10–600s), BPM, key/scale, time signature, genre tags, 1000+ instrument/timbre tags. **Correction**: this doc previously attached "50+ languages" to the text *prompt* — the real vocabulary lives on `lyrics`/`vocal_language` instead (`servers/ace-step-1.5/vendor/acestep/constants.py`'s `VALID_LANGUAGES`, 49 ISO codes + `'unknown'`; `docs/en/API.md`: `vocal_language | string | "en" | Lyrics language`), not the prompt, which is unrestricted free text. The real API also exposes `use_cot_language` (5Hz-LM chain-of-thought auto-detection of `vocal_language` — `acestep/llm_inference.py`), not currently exposed in this app's manifest — this app spawns the server with `ACESTEP_INIT_LLM=false` for a faster/lighter startup, so the LLM auto-detect path can't actually run yet, and building a UI control for it would silently no-op. **`vocal_language` is now a real manifest field** (`isModelLanguageField`, `src/data/manifests.ts`): a fixed `select` populated from the exact `VALID_LANGUAGES` vocabulary (`src/data/languages.ts`, verified byte-for-byte against `constants.py`), auto-seeded from the selected artist's primary language (`ArtistProfile.languages`, set in Settings → Artists) the same way genre auto-populates. YuE2's own language concept is unstructured (folded into its free-text `style` field per its docs) — it gets the same `isModelLanguageField` flag on a plain `text` input instead, pre-filled with the artist's primary language by name. Model Manager shows a "Multilingual" pill only for models whose manifest declares this flag (ACE-Step, YuE2 today) — every other model (MusicGen, MuseCoco, Museformer, RAVE) shows nothing, since none has a real language concept.
...
- Inputs: lyrics text, style/genre spec (folded together at generation time), optional reference audio (WAV/MP3) for cover/transcription, VAE decoder choice (`yue2-vae` or `yue2-vae-legacy`). A `max_seconds` UI field exists but is **not yet wired to a real pipeline constraint** — the real length knob is a semantic-token budget with no documented tokens-per-second rate, so the field is left as a future-phase hook.
- Outputs: **dual** — stereo audio (`audio.flac`, 48 kHz) **and** ABC notation (`score.abc`). The app surfaces both: the existing audio player for FLAC and `AbcScoreViewer` for the ABC text. Other reproducibility artifacts (LAB annotations, `.npy` intermediates) are written by the pipeline but not shown in the UI.
- Hardware: 24GB+ NVIDIA VRAM (BF16), Linux, batch only — heaviest model in the catalog. A real Phase 8 standalone smoke test peaked at only ~3-4GB observed VRAM for one short song, but the published 24GB minimum is kept as the manifest floor.
...
- **App integration status**: wired into `electron/models/modelServer.ts` via a hand-written FastAPI wrapper (`servers/yue2/server.py`) following MusicGen/MuseCoco's pattern. The venv install is automated in Settings → Models / Environments. A real standalone generation was proven (34.6s GPU compute for ~60s of audio, verified non-silent). A full end-to-end run through the Electron UI has not been exercised because the YuE2 model weights are not present on this machine.
...
- Checkpoints: small (300M), medium (1.5B), large (3.3B), melody (1.5B, melody-conditioned). **`style` removed from the catalog**: `facebook/musicgen-style`'s real checkpoint needs a `StyleConditioner` class that does not exist in the pinned `audiocraft==1.3.0` (confirmed by grepping the installed package directly) — selectable but guaranteed to fail, so it was removed (manifest, seed data, download script) rather than shipped broken, with a one-time DB/disk migration (`removeDiscontinuedMusicGenStyleVariant` in `electron/db/database.ts`) cleaning up any already-installed copies.
...
- Inputs: free text description **plus** structured attributes — instrument, genre, mood, tempo, key, time signature, bar count, rhythm/danceability, pitch range, artist style. Richest structured-input model in the catalog. **Phase 7 correction**: the real attribute vocabulary (reverse-engineered from the vendored inference code, not documented anywhere in the repo's README) is narrower/differently-shaped than assumed here in several places — key only distinguishes major/minor, pitch range means octaves spanned not register, mood is a 4-quadrant valence/arousal model not free text, artist style is a fixed 17-classical-composer list not free text. Full table in `servers/musecoco/README.md`. **Real bug found and fixed while adding artist-profile genres**: `genre` was a free-text comma-separated field, but the real server (`match_categories()`) expects a JSON array and iterates whatever it's given — a string iterates as characters, so real genre conditioning silently matched nothing no matter what was typed. Now a real fixed 22-value multiselect (the app's genre catalog auto-populates any that overlap with the selected artist's own genres); `instrument` has the identical bug, still open, flagged separately. See `servers/musecoco/README.md`'s "A real bug" note.
...
- Inputs: random/unconditional generation, or MIDI seed/continuation via `seed_mode` / `seed_midi` — no free-text prompt. See `servers/museformer/server.py` and `servers/museformer/README.md` for the real, REMIGEN2-based MIDI primer implementation.
...
- Hardware/Python: Python 3.8, `fairseq==0.10.2` (same era/pin as MuseCoco) confirmed from the real vendored code in Phase 7. **Inference verified end-to-end on GPU** — a fresh venv built only from `servers/museformer/requirements.txt` produced a real, valid, non-empty `.mid` file (127 note events, verified with `mido`) through the real `fairseq-interactive` decoder. See `servers/museformer/README.md` for the full verification story, including three kernel workarounds and the real MIDI-seeded continuation test. **CPU is not supported**: the checkpoint's default `attention_impl='blocksparse'` uses Triton kernels with no CPU backend (`--cpu` hard-fails at launch).
...
- Hardware: training 5–32GB VRAM depending on config; inference/streaming much lighter, CPU-feasible for small models in realtime form. **Confirmed Phase 9**: batch-mode inference on CPU is genuinely fast — a standalone timing test against the 26MB `darbouka_onnx` checkpoint encoded+decoded 3 real seconds of audio in well under half a second.
- Python: PyTorch + torchaudio (no longer strictly version-pinned), FFmpeg required on the host. **Confirmed Phase 9, narrower than this line implies for batch inference specifically**: each of the nine pretrained checkpoints already downloaded (`models/rave/<name>/<name>.ts`) is a self-contained TorchScript export — `torch.jit.load()` plus `.forward()`/`.encode()`/`.decode()` is the entire real inference surface, verified by loading one with nothing installed but `torch` itself. Batch-mode `servers/rave/server.py` needs only `torch`, `torchaudio`, and `soundfile` — no `acids-rave` pip install, no vendored repo. (The `acids-rave` package itself is still the right dependency for training/fine-tuning or `rave export --streaming`, neither of which this finding touches.) **Also confirmed**: none of the nine `.ts` files carry sample-rate metadata (checked directly — no `extra_files`, no exported `sr` attribute, nothing in `constants.pkl`); `server.py` defaults to 44100Hz, IRCAM/ACIDS's own documented rate for most pretrained examples, since the model-zoo page's real per-checkpoint rates can't be scraped (see the row above). See `servers/rave/README.md` for the full writeup, including two real per-checkpoint bugs found and fixed (a stereo checkpoint's channel ordering, and a decoder whose output isn't bounded to `[-1, 1]`).
